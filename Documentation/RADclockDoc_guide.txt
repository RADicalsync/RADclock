...
are covered below). It is useful to view the documents with colour settings set to C syntax.

There are also the man pages in repo/man and repo/kernel/freebsd/FreeBSD-XX/CurrentSource
and in the system once installed :
...
packets.  If these are not available, it can `run dead` to synthesize
...
advantage of a defined `kernel bypass mode` when available, to access counter
...
are merged upstream into FreeBSD from 13.0).
...
We recommend the use of  update_FFkernelsource  as it will work directly on top
of any other FFclock patched release that may already be installed.  It also
installs the header files in  /usr/include  to help in application recompilation
if needed.
...
(Note this is not a FFclock `feature`, it has been in FreeBSD since around
release 11.0)
...
These two version of the TSC are equivalent in terms of value (thanks to
...
Without kernel support, radclock can still compile and run `dead` on stored
data traces, but cant run live or act as a daemon for the system clock.
...
# Timestamps obtained through standard system clock read functions
	...
	** See warning below.
	...
	This option is definately not recommended, this `radclockd` replacement to
...
# Timestamps obtained via pcap, interpreted through radclock
	...
	This timestamping occurs in the kernel and so has much lower latency `noise`
...
The API allows programs to define `user radclocks`, enabling radclock reading
...
The FF-aware kernel expands the system clock or `SYSclock` architecture.
