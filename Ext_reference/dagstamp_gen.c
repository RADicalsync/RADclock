...
struct dag_cap {
	...
	struct in_addr ip;      // IP address of the NTP reply server
};

struct dag_ntp_record {
	int key_id;             // Key in outgoing request, the returning auth key must match
	struct in_addr IPout;   // IP of outgoing NTP packet, used for filtering incoming NTP responses
	struct dag_cap cap;
};

/* This "list" is really a buffer or processing/pre-matching queue it seems, better comment block needed */
struct rec_list {
...
/* Same as in dag_extract.c
 * Converts the unsigned long long timestamp from the DAG card to seconds and ns.
...
// TODO:  remove this.  Note off_t ends up being __int64_t on amd64
off_t tell(int fd)
{
...
/* First bit the same as in dag_extract.c, but with very different arguments, and functionality!
 * Originally "Output formatting for NTP packets."
 *  - in fact not only formats, but does packet matching! and writes the formatted NTP data.
 * TODO: rename variables and structures to make this a lot clearer.
 */
static long int
format_ntp(dag_record_t *header, void *payload, struct stats_t *stats,
    struct rec_list *cap_list, char *cn_ip, int s_server, struct sockaddr_in s_to, char **key_data)
{
	struct timespec ts;
	...
	// TODO:  remove unused bulky isSrc, unless useful elsewhere
	int isSrc = 1;
	...
	if ((strcmp(dst, cn_ip) == 0 || strcmp(src, cn_ip) == 0) && port == PORT_NTP ) {
		...
		printf("Packet cap %s -> %s\n", src, dst);  // DV: debugging verb
		int key_id = -1;
		if (check_signature(packet_size, ntp, key_data, &auth_bytes, &key_id)) {
			if (strcmp(src, cn_ip) == 0) {	// Use isSrc?
				thisrec = &cap_list->recs[cap_list->write_id];  // current rec
				thisrec->key_id = key_id;
				memcpy(&thisrec->IPout, &hdr_ip->ip_src, sizeof(thisrec->IPout));
				thisrec->cap.Tout = ts.tv_sec + (long double)(ts.tv_nsec * 1e-9);
				thisrec->cap.server_reply_org = ntp->xmt;
				cap_list->write_id = (cap_list->write_id + 1) % cap_list->size;
			} else {
				long double Tin;
				Tin = ts.tv_sec + (long double)(ts.tv_nsec * 1e-9);

				/* Look for a match of outgoing and incoming IP's */
				int match_id = -1;
				for (int i = 0; i < cap_list->size && match_id == -1; i++) {
					thisrec = &cap_list->recs[i];
					if (memcmp(&ntp->org, &thisrec->cap.server_reply_org, sizeof(ntp->org)) == 0)
						if (key_id == thisrec->key_id) // same auth method must be present in sent and rcv ntp packets
							if (memcmp(&hdr_ip->ip_src, &thisrec->cap.ip, sizeof(hdr_ip->ip_src)) == 0)
								match_id = i;	// record a match  [ will overwrite previous one.. ]
				}

				if (match_id != -1) {
					thisrec = &cap_list->recs[match_id];  // matched rec
					thisrec->cap.Tin = Tin;
					thisrec->cap.server_reply_org = ntp->org;
					...
					printf("%d: (%ld.%09ld %.09Lf) %d\n\n", match_id, ts.tv_sec,
					    ts.tv_nsec, Tin, ntohs(hdr_udp->uh_ulen));
					...
					memset(&thisrec->cap.server_reply_org, 0, sizeof(ntp->org));
				} else {
					long double org_ts = NTPtime_to_UTCld(ntp->org);
...
static int 
write_record(dag_record_t *header, void *payload, struct stats_t *stats,
    struct rec_list *cap_list, char *cn_ip, int s_server, struct sockaddr_in s_to, char **key_data)
{
    ...
    struct rec_list *cap_list, char *cn_ip, int s_server, struct sockaddr_in s_to, char **key_data)
{
	...
	while(1) {
		int length = tell(istream->fd);	// TODO: tell is obscure, replace with actual code + comment
...
{
	cap_list->recs = malloc(sizeof(struct dag_ntp_record) * PKT_LIST_SIZE);
	cap_list->size = PKT_LIST_SIZE;
	cap_list->write_id = 0;

	// Set all the rcv time stamps to 0 to avoid incorrect matches
	// TODO: why only receive timestamps? shy not the entire structure.
	for (int i = 0; i < PKT_LIST_SIZE; i++)
		memset(&cap_list->recs[i].cap.server_reply_org, 0,
		    sizeof(cap_list->recs[i].cap.server_reply_org));
}
...
{
	if (argc < 3) {
		printf("Usage:  shm_dag  cap.erf  TN_IP\n");
