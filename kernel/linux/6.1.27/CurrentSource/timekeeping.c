 ...
 *   initialization:     [ flagged by ncount = 0, ffclock_bootime = 0 ]
 ...
 * ffclock_bootime:
 *   - no effort is made to align to the FB bootime variable, the
 *     FFclock Up variants are based on the FFclocks only
 *   - non-trivially initialised here, on the invocation passing a _UTC arg
				...
				const struct timespec64 *delta_UTC)
{
	...
	if (reset_UTC || delta_UTC) {
		 ...
		 * TODO: don't want this if you suspended.. how to tell cause of reset? */
		if (bintime_isset(&ffclock_boottime)) {
			ffth->tick_time_lerp = fftimehands->tick_time_lerp;
			bintime_clear(&gap);
		...
		} else {   // bootime not yet set, initialize it to now
			ffclock_boottime = reset_bin;
			printk("FFclock initializing bootime to: %lld.%03lu\n",
			    (long long)ffclock_boottime.sec,
		...
		upt = ffth->tick_time_lerp;
		bintime_sub(&upt, &ffclock_boottime);
		printk("FFclock processing reset: UTC: %lld.%03lu"
			" boottime: %lld.%03lu, uptime preserved at: %lld.%03lu\n",
			(long long)ffth->tick_time_lerp.sec,
			(long unsigned)(ffth->tick_time_lerp.frac / MS_AS_BINFRAC),
			(long long)ffclock_boottime.sec,
			(long unsigned)(ffclock_boottime.frac / MS_AS_BINFRAC),
			(long long)upt.sec,
			(long unsigned)(upt.frac / MS_AS_BINFRAC) );
