...
#define SAMPLE_INTERVAL_MS 10 // Sample ring buffer every 10ms
 ...
 * This seems buggy. Replacing with reliable fn usleep previously used in radclock
 */
...
void packet_callback_msgpuck(void * data, int packetId, int dataSize, long double timestamp)
{
	...
	for (int i = 0; i < ocn_data->NTC_Count; i++) {
		NTC_data =  (Radclock_Telemetry_NTC_Latest *)(data +
			sizeof(Radclock_Telemetry_Latest) +
			sizeof(Radclock_Telemetry_NTC_Latest)*i);
		char ntc_name[128];
...
int pull_telemetry_batch(int *ring_read_pos, int *prior_packet_id,
    void * shared_memory_handle, int *packets_recorded, int priorId,
    int *holding_buffer_size, void* holding_buffer, int prior_packet_size )
{
	...
	if (prior_packet_size < 1) {
		...
		read_bytes += ring_buffer_read(&data.header,
			sizeof(Radclock_Telemetry_Header), shared_memory_handle, ring_read_pos, 1);
		if (read_bytes < 1)
		{
		...
		read_bytes2 = ring_buffer_read(holding_buffer + sizeof(Radclock_Telemetry_Header),
			data.header.size - sizeof(Radclock_Telemetry_Header), shared_memory_handle, ring_read_pos, 0);
		if (read_bytes2 < 1) {
		...
		if (prior_packet_size!= read_bytes)
			verbose(LOG_ERR, "Telemetry Consumer - prior_packet_size error2 %d %d %d %d",
			    prior_packet_size, read_bytes, starting_ring_read_pos%RADCLOCK_RING_BASE_BUFFER_BYTES,
			    RADCLOCK_RING_BASE_BUFFER_BYTES);
	}
	...
	{
		verbose(LOG_ERR, "Telemetry Consumer - Error packet bytes: %d different from packet.header.size: %d (%d, %d)",
			read_bytes, new_data.header.size, read_bytes-read_bytes2, read_bytes2);
		if (EXIT_ON_TERMINAL_ERROR)
			exit(0);
	}

	if (RADCLOCK_TELEMETRY_VERSION != new_data.header.version) {
		verbose(LOG_ERR, "Telemetry Consumer - Error packet version: %d is different from expected: %d",
			new_data.header.version, RADCLOCK_TELEMETRY_VERSION);
		if (EXIT_ON_TERMINAL_ERROR)
			exit(0);
	}
	...
	if (current_packet_id != *prior_packet_id + 1) {
		...
		if (get_prior_packet_available(shared_memory_handle, starting_ring_read_pos, &prior_packet_read_pos, &new_packet_size))
			pull_telemetry_batch(&prior_packet_read_pos, prior_packet_id, shared_memory_handle,
			packets_recorded, priorId+1, holding_buffer_size, holding_buffer, new_packet_size);
	}

	if (TELEMETRY_CONSUMER_LOG_VERBOSITY && new_data.header.packetId % TELEMETRY_CONSUMER_LOG_PACKET_INTERVAL == 0)
		verbose(LOG_ERR, "Telemetry Consumer - Client: rp:%d prior:%d packet:%d v:%d picn:%d size:(%d, %d) (%d %d %4.2f%%)",
			*ring_read_pos, priorId, new_data.header.packetId, new_data.header.version,
			new_data.PICN, new_data.header.size, read_bytes, new_data.header.packetId,
			*packets_recorded, (double)(*packets_recorded) *100.0 / (double)new_data.header.packetId);
	*packets_recorded += 1;
